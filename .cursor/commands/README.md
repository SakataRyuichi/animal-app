...
スキーマを更新したあと、関連ドキュメント（ER図など）の更新漏れを防ぐために実行します。スキーマ変更時に「ER図や README を更新したか」をチェックし、必要なら更新を促します。

詳細は [schema-docs-sync.md](./schema-docs-sync.md) を参照してください。
...
- **スキーマ関連ドキュメントの更新漏れチェック**（オプション）: `docs/schema/` のみ変更して `docs/diagrams/` を変更していない場合、コミットを阻止し `/schema-docs-sync` を促す
