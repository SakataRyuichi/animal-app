...
**関連ドキュメント**:
- [CONVEX_SCHEMA.md](../schema/CONVEX_SCHEMA.md): imagesテーブルの定義
- [PREMIUM_FEATURES.md](./PREMIUM_FEATURES.md): 画像管理のプレミアム機能
- [USER_STORIES.md](../stories/USER_STORIES.md): US-051〜US-054（画像管理機能）、US-092〜US-095（動画管理機能） ✅ **2026年追加**
...
- WebP形式（圧縮後）:
  - 表示用（Preview）: 約250KB 〜 500KB（幅1080px、Quality 0.6-0.7）
  - 最高画質（Original）: 約1.5MB 〜 3MB（リサイズなし、Quality 0.9-1.0）

**1ユーザーあたりの月間データ量**（1日1枚、月30枚と仮定）:
...
- **プレミアムユーザー**:
  - 編集前（最高画質）
  - 編集後（表示用WebP）
  - 編集メタデータ（スタンプの位置や文字の内容）
  - 後からスタンプを動かしたり文字を書き換えたりできる「非破壊編集」
...
### 4.3 activitiesテーブルの更新

`payload.images`を`imageIds`（`images`テーブルのID配列）に変更します。
...
/**
 * 画像を2種類のWebPに変換
 * 1. 表示用（Preview）: 幅1080px、Quality 0.6-0.7
 * 2. 最高画質（Original）: リサイズなし、Quality 0.9-1.0
 */
...
export const uploadImage = action({
  args: {
    ...
    previewFile: v.string(), // Base64またはBlob
    originalFile: v.string(), // Base64またはBlob
    width: v.number(),
    height: v.number(),
  ...
  handler: async (ctx, args) => {
    ...
    // 無料ユーザーの場合、画像枚数制限をチェック
    if (user.subscription.tier === "free") {
      const FREE_IMAGE_LIMIT = 50; // 無料ユーザーは累計50枚まで
    ...
    <View>
      ...
      {!isPremium && (
        <Text style={styles.hint}>
          プレミアムに入ると、{Math.round(image.fileSizeOriginal / 1024 / 1024 * 10) / 10}MB の最高画質で表示・保存できます
        </Text>
      )}
      {!isPremium && image.isPremiumAtUpload && (
        <Text style={styles.hint}>
          💎 この写真は最高画質で保存されています。プレミアムで表示できます
        </Text>
      )}
    </View>
  );
...
1. **編集画面を開く**: ユーザーが画像を選択し、編集画面を開く
2. **スタンプ・文字を追加**: Expoの`expo-pixi`や`react-native-canvas`を使用
3. **編集完了**: `view-shot`ライブラリで編集後の画像をWebPとして書き出し
4. **保存**: 
   - **無料ユーザー**: 編集後の画像のみ保存（編集前は削除）
   - **プレミアムユーザー**: 編集前・編集後の両方を保存、編集メタデータも保存

### 7.2 編集メタデータの保存

編集メタデータを保存することで、後から編集をやり直したり、スタンプを動かしたりできます。

```typescript
// 編集メタデータの例
{
  originalBeforeEditStorageId: "storage_xxx", // 編集前のオリジナル
  stamps: [
    {
      type: "crown",
      x: 100,
...
- 最高画質WebP（ウォーターマークなし）をダウンロード可能
- 編集前の「素」の状態のオリジナルデータもダウンロード可能
- 一括ダウンロード機能（1ヶ月分をまとめて保存など）
...
- AIチャット機能は利用不可（または1日1回のみ）
- 代わりに、コラム・記事の閲覧を推奨

### 9.2 プレミアムユーザー

- AIチャット機能を無制限に利用可能
- ペットの過去1週間のログ（食事量、便の様子など）をAIに読み込ませた上で回答
- 「うちの子専用のアドバイザー」化

### 9.3 コスト管理

- 簡単な質問は安価なモデル（Claude Haikuなど）で処理
- 複雑な質問は高価なモデル（GPT-4o、Claude Opusなど）で処理
- ルーティング設計でコストを最適化
...
無料ユーザーでも画像はアップできるが、50枚を超えると：
- 「古い写真からぼかし（Blur）がかかり、プレミアムになると復元できる」という見せ方
- 「データは消していない（温かみ）」と「今すぐは見られない（制限）」を両立

### 10.2 「AIからの招待」

ペットの体調記録が3日続いたら、AIから：
- 「3日間頑張りましたね！プレミアムなら、この記録からレオくんの健康診断レポートを作れますよ」と、文脈に沿った提案を出す

### 10.3 視覚的な案内

- プレミアム機能の横に小さな鍵アイコンを表示
- 画像を拡大した際、「プレミアムなら Ultra HD で表示・ダウンロードできます」という案内を出す
- 無料ユーザーの画面では、写真の隅に「SD（標準画質）」という小さなラベルを表示
...
- `CONVEX_SCHEMA.md`: スキーマ定義（`images`テーブル、`users.subscription`）
- `USER_STORIES.md`: ユーザーストーリー（画像関連: US-051〜US-054、AI相談: US-020〜US-025）
- `PREMIUM_FEATURES.md`: プレミアム限定機能リスト
- `packages/backend/convex/lib/permissions.ts`: 権限管理ヘルパー関数
- `packages/backend/convex/lib/imageLimits.ts`: 画像制限管理ヘルパー関数
